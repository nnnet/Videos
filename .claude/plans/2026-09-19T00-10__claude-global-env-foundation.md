...
graph:
  ...
  - {id: D5, needs: [D4],         parallel: "",           status: "[~]", files: [ViralMint/.claude/skills/sdd-verify/**, ~/.bashrc]}
...
Закрыто шестнадцать узлов: **A1, A2, A3, B1, B2, B3, B4, C1, C3, D1, D2,
D3, D4, G1, G2, G4**. Событийный слой работает на живых данных этой сессии;
...
|---|---|---|
| D5 `sdd-skill` | агент | `[~]` — файл написан, половина приёмки упёрлась в квоту прокси; ждёт одной проверки человеком |
| F1 `decision-capture` | агент | нет — следующий; зона `.repowise/config.yaml` освободилась с закрытием C1 |
